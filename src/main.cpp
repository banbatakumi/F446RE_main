            ...
            if (curveShootTimer.read() > 0) {   // 後ろドリブラーのマカオシュート
                  ...
                  if (camera.front_goal_size > 25 && abs(camera.front_goal_dir) < 15) {   // キッカーを打つ条件
                        ...
                        if (lidar.val[lidar.FrontMinSensor()] < 50 && (abs(camera.front_goal_dir) < 30 || camera.front_goal_size == 0)) {
                              ...
                              motor.Run(camera.front_goal_dir * 2, 75);
                        }
                  }
            } else if (holdBack.IsHold()) {   // 後ろに捕捉している時
                  if (camera.front_goal_size > 30 && abs(camera.front_goal_dir) < 30) {   // カーブシュート開始
                  ...
                  if (abs(camera.ball_dir) < 150) {   // 前の捕捉エリアに回り込む
                        ...
                        wrapDisPID.Compute(camera.ball_dis, 150);
...
void Ui() {
      ...
      if (data_length == 0) {
            ...
            item = uiSerial.getc() - 100;
            data_length += 1;
      } else if (data_length == 2) {
            mode = uiSerial.getc();
            data_length += 1;
      } else if (data_length == 3) {
            is_own_dir_correction = uiSerial.getc();
            data_length += 1;
      } else if (data_length == 4) {
            moving_speed = uiSerial.getc();
            data_length += 1;
      } else if (data_length == 5) {
            line_moving_speed = uiSerial.getc();
            data_length += 1;
      } else if (data_length == 6) {
            dribbler_sig = uiSerial.getc();
            ...
            if (voltage.IsLowVoltage() == 1) {
                  ...
                  if (item == 0) {
                        ...
                        int16_t debug_val_3 = sensors.encoder_val[2];
                        int16_t debug_val_4 = sensors.encoder_val[3];
