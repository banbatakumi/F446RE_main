...
Motor::Motor(PinName motor_1_a_, PinName motor_1_b_, PinName motor_2_a_, PinName motor_2_b_, PinName motor_3_a_, PinName motor_3_b_, PinName motor_4_a_, PinName motor_4_b_, float *own_dir_)
      ...
      for (uint8_t i = 0; i < MOTOR_QTY; i++) {
            ...
            if (abs(power[i]) >= 10 && readms(addPowerTimer) < 100) {
                  ...
                  if (encoder_val[i] <= abs(corrected_power[i]) / (100.0f / BASE_POWER)) {
                        add_power[i] += (d + pre_d) * readus(addPowerTimer) * 0.5 * ENCODER_GAIN;  // 台形積分
